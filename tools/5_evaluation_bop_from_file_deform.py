...
for scene_image_id, detections in data.items():
    ...
    if sys.argv[3] == "itodd":
        ...
        image_path = f"{scene_id:06d}/rgb/{image_id:06d}.jpg"
...
def get_gt_detection(path):
    ...
    if sys.argv[3] == "tless":
        ...
        relative_image_path = path.split('train_pbr/')[1]

    print(relative_image_path)
    #print(image_detection_map)
    ...
    elif sys.argv[3] == "deform_dataset":
        obj_orders = (np.ceil(obj_ids/404)-1).astype(int)
    else:
...
test_dir = os.path.join("/dataset", "deform_dataset", "train_pbr")
