...
def augment_inplane_gen(xyz_id, img, img_r, depth_rend, mask, isYCB=False, step=10):
    ...
    for rot in np.arange(step, 360, step):
        ...
        if vu_box[0].size == 0 or vu_box[1].size == 0:
            continue
        
        bbox_t = np.array([np.min(vu_box[0]), np.min(vu_box[1]), np.max(vu_box[0]), np.max(vu_box[1])])
        if isYCB:
            img_npy = np.zeros((bbox_t[2] - bbox_t[0], bbox_t[3] - bbox_t[1], 7), np.uint8)
        else:
            img_npy = np.zeros((bbox_t[2] - bbox_t[0], bbox_t[3] - bbox_t[1], 6), np.uint8)
...
for m_id,model_id in enumerate(model_ids):
        ...
        if(t_model!=-1 and model_id!=t_model):
            continue
        m_info = model_info['obj_{:06}'.format(model_id)]
        ...
        xyz_dir =xyz_target_dir+"/{:02d}".format(int(np.ceil(model_id/404)))
        ...
        for img_id in range(len(rgb_files)):
            ...
            #gt = (gts[img_id])[0]
                ...
                if(obj_id !=int(model_id)):
                    continue            
                xyz_fn = os.path.join(xyz_dir,"{:07d}.npy".format(xyz_id))
                ...
                #     enlarged_bbox_size = int(max(bbox_width, bbox_height) * 1.3)
