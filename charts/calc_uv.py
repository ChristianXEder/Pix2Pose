...
for scene_number in sorted(os.listdir(os.path.join(bop_dir,'train_pbr'))):
    ...
    for image_name in sorted(os.listdir(os.path.join(bop_dir,'train_pbr',scene_number,'rgb'))):
        ...
        for index, obj in enumerate(scene_bb['{}/{}'.format(scene_number, image_number)]):
            ...
            mask_all = np.all(rgb_mask_image > 0, axis=-1)

            rgb_error_mask = np.mean(np.square(np.subtract(img_org[mask_pred], rgb_mask_image[mask_pred])),axis=0)
            rgb_error_all = np.mean(np.square(np.subtract(img_org[mask_all], rgb_mask_image[mask_all])),axis=0)

            results.append({"scene":scene_number, 
                                "img": image_number,
                                "obj":cat_list[new_obj_id],
                                "rgb_error_mask":list(rgb_error_mask),
                                "rgb_error_all":list(rgb_error_all)})

    # Writing to sample.json
    with open(os.path.join(bop_dir,'rgb_error','{}.json'.format(scene_number)), "w") as outfile:
